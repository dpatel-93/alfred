...
// --- modelFamily() classification sanity (same rule as jarvis/server.mjs) ---
...
// --- queryUsage: groups by at least two dimensions on a small synthetic ledger ---
{
  ...
  const rows = [
    ...
    buildSpendRecord({ date: today, agent: 'agentB', model: 'claude-opus-4-1', project: 'Tickr',
  ...
  chk('queryUsage groupBy project sums per project', (byProject.find((g) => g.key === 'Tickr') || {}).costUsd === 5,
...
// --- rollUpBySpawnTree: 3-level chain root -> child -> grandchild ---
{
  ...
  appendSpendRecords([
    ...
    buildSpendRecord({ date: today, agent: 'main', agentId: null, parentAgentId: null, spawnDepth: null,
      model: 'claude-sonnet-4-5', project: 'Tickr', inputTokens: 1_000_000, outputTokens: 0,
