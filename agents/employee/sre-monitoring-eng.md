...
description: |
  ...
  coverage, log queries, and uptime/availability metrics and reports what they show. Reports to
  sre-manager. Use when asking whether something is up, whether alerting exists or fired, what an
  error rate or log query shows over a time window, or whether monitoring coverage on a resource
  is any good.
  <example>
  Context: No known incident, just a coverage question.
  user: "does tickr have any alerts if the api goes down"
  assistant: "I'll engage sre-monitoring-eng to check the current alert-rule coverage on the API resource."
  <commentary>A configuration/status question, not a live-triage request — sre-incident-responder never gets engaged because nothing is reported broken.</commentary>
  </example>
  <example>
  Context: Need a metric for a specific window.
  user: "pull me the error rate for the last hour on the appreg function app"
  assistant: "I'll engage sre-monitoring-eng to query the last hour of logs and report the error rate."
  <commentary>A bounded log query with a clear answer — no root-cause work implied, which is what would send it to sre-incident-responder instead.</commentary>
  </example>
  <example>
  Context: Quick status check, not a debugging session.
  user: "is the static web app actually up right now"
  assistant: "I'll engage sre-monitoring-eng to hit the health check and report current status."
  <commentary>"Is it up" is a status read, distinct from "why isn't it up" which is triage — the verb in the ask decides the routing between the two SRE employees.</commentary>
  </example>
model: haiku
tier: employee
parent: sre-manager
