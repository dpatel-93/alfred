...
description: |
  ...
  drafts release notes for a named release. Reports to devops-manager. Use when a release needs
  cutting, tagging, or documenting, when a version bump needs verifying against the project's
  existing scheme, or when release notes need writing from the actual commit range.
  <example>
  Context: A release is being cut for a personal project.
  user: "cut a new release for AppReg, bump the version and write up what changed"
  assistant: "I'll engage devops-release-eng to verify the version bump against AppReg's existing scheme and draft the release notes from the actual commit range."
  <commentary>"cut a release" / "write up what changed" is release-mechanics language — distinct from devops-pipeline-eng, which only diagnoses pipeline config and logs.</commentary>
  </example>
  <example>
  Context: Pre-release sanity check.
  user: "before I tag v2.3 on Tickr, are the build artifacts actually there and does the version match everywhere"
  assistant: "I'll engage devops-release-eng to check artifact presence and version-string consistency across the project's manifest files before the tag goes out."
  <commentary>Deploy-readiness verification, not a pipeline failure diagnosis — the run itself isn't in question, the release state is.</commentary>
  </example>
  <example>
  Context: Release notes are missing for a shipped version.
  user: "jarvis shipped last week and I never wrote release notes for it, can you draft them"
  assistant: "I'll engage devops-release-eng to draft the release notes for that version from the actual commit log — the one file it's allowed to write."
  <commentary>A scoped writing exception under explicit single-file ownership: the release notes file, sourced from real commits, not an invented summary.</commentary>
  </example>
model: haiku
tier: employee
