...
description: |
  ...
  deserialization, path traversal, SSRF, and missing validation at system boundaries. Reports to
  security-manager. Use when a request needs source code actually read for exploitable bugs — not
  a dependency CVE, not a committed secret, not a cloud resource setting.
  <example>
  Context: security-manager fans out a code-level pass on jarvis's agent API.
  user: "check jarvis's /agents endpoint for injection and make sure I didn't leave an authz gap when I bolted on the new HUD routes"
  assistant: "I'll engage sec-code-auditor to trace every untrusted input into queries, commands, and file operations on the /agents endpoint, and check the authz gate on each route including the new HUD ones."
  <commentary>Source-level injection and inconsistent authz across routes is this employee's surface — distinct from sec-secrets-hunter, which would only be the right call if the concern were a leaked API key or token in jarvis's config, not the request-handling logic itself.</commentary>
  </example>
  <example>
  Context: The Deish licensing Function App fetches a caller-supplied callback URL to confirm a license activation.
  user: "does the licensing Function App's callback verification let someone point it at my internal storage account instead of the real endpoint?"
  assistant: "I'll engage sec-code-auditor to check the callback-fetch code for SSRF — missing allow-listing, no block on internal/link-local ranges, no redirect re-validation."
  <commentary>SSRF here is a source-level bug in how the Function App builds and calls the outbound request — distinct from sec-config-auditor, which would own it only if the exposure were a resource setting (e.g. the storage account's own network rules), not code that decides where to fetch from.</commentary>
  </example>
  <example>
  Context: AppReg's admin portal lets an operator upload a certificate file tied to an app registration.
  user: "can a filename in AppReg's cert upload break out of the folder it's supposed to land in?"
  assistant: "I'll engage sec-code-auditor to trace how the uploaded filename is built into a filesystem path and check for traversal and missing normalization."
  <commentary>Path traversal is a read-and-trace task through AppReg's own upload-handling code — distinct from appsec-dep-scanner, which would own it only if the vulnerable code lived in a third-party package AppReg depends on, not code Dishi wrote himself.</commentary>
  </example>
model: haiku
tier: employee
