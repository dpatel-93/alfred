...
const handlers = {
  'route': () => {
    ...
    // NOTE: UserPromptSubmit stdout is injected into the model's context on
    // every prompt. Keep this SILENT unless there is genuine signal to add —
    // do NOT emit fabricated routing tables/metrics (they waste tokens and
    // mislead the model). The real keyword router below is advisory only; its
    // result is intentionally not printed. Genuine context is added by
    // intelligence.getContext() above when it has something to say.
  },

  'pre-bash': () => {
    // Basic command safety check — prefer stdin command data from Claude Code.
    // Per Anthropic Claude Code hook spec: PreToolUse exit code 2 = block + show
    // stderr to model. Exit 1 is a generic hook error that does NOT block.
