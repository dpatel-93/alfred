...
const DATA_DIR = join(PROJECT_ROOT, '.alfred-flow', 'data');
...
async function loadMemoryPackage() {
  ...
  const localDist = join(PROJECT_ROOT, 'v3/@alfred-flow/memory/dist/index.js');
  ...
  // when installed as a transitive dependency via npx alfred / npx alfred-flow)
  try {
    const { createRequire } = await import('module');
    const require = createRequire(join(PROJECT_ROOT, 'package.json'));
    return require('@alfred-flow/memory');
  } catch { /* fall through */ }

  // Strategy 3: ESM import (works when @alfred-flow/memory is a direct dependency)
  try {
    return await import('@alfred-flow/memory');
  } catch { /* fall through */ }

  // Strategy 4: Walk up from PROJECT_ROOT looking for @alfred-flow/memory in any node_modules
  let searchDir = PROJECT_ROOT;
  const { parse } = await import('path');
  while (searchDir !== parse(searchDir).root) {
    const candidate = join(searchDir, 'node_modules', '@alfred-flow', 'memory', 'dist', 'index.js');
...
// ============================================================================
// Read config from .alfred-flow/config.yaml
// ============================================================================

function readConfig() {
  const configPath = join(PROJECT_ROOT, '.alfred-flow', 'config.yaml');
  const defaults = {
