 ...
 * the hook system. Pure CJS — no ESM imports of @alfred-flow/memory.
 *
 * Data files (all under .alfred-flow/data/):
...
const DATA_DIR = path.join(process.cwd(), '.alfred-flow', 'data');
const STORE_PATH = path.join(DATA_DIR, 'auto-memory-store.json');
const GRAPH_PATH = path.join(DATA_DIR, 'graph-state.json');
const RANKED_PATH = path.join(DATA_DIR, 'ranked-context.json');
const PENDING_PATH = path.join(DATA_DIR, 'pending-insights.jsonl');
const SESSION_DIR = path.join(process.cwd(), '.alfred-flow', 'sessions');
 ...
 * on @alfred-flow/memory for the initial seed.
 */
function bootstrapFromMemoryFiles() {
  ...
  const candidates = [
    ...
    path.join(cwd, '.alfred-flow', 'memory'),
