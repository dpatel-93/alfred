...
const stub = fs.mkdtempSync(path.join(os.tmpdir(), 'alfred-verify3-'));
...
// Force one seat "not ready" by directly mutating the module-scope ccSeats
// via the debug hook path isn't exposed, so patch the DOM-visible state by
// re-fetching seats with an injected fetch override is overkill for a smoke
// check -- instead, directly flip ccChecked/style is not representative.
// Simplest honest check: read the actual CSS rule effect by adding the class
// manually (exercises the SAME selector renderCcSeats would apply) and
// confirm dimming + non-hidden, matching plan's "never hidden" requirement.
const info = await page.evaluate(() => {
  var row = document.querySelector('#cc-seats .seat[data-seat-id="dsh"]');
  var before = { display: getComputedStyle(row).display, opacity: getComputedStyle(row).opacity };
  row.classList.add('idle');
  var after = { display: getComputedStyle(row).display, opacity: getComputedStyle(row).opacity, visible: row.offsetParent !== null };
  return { before, after };
});
