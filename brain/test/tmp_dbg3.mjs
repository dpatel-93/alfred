...
await page.waitForFunction(() => document.querySelectorAll('#cc-tabs .cc-tab').length === 2, null, { timeout: 10000 }).catch(() => {});
await page.click('#cc-grid-toggle');
await page.waitForFunction(() => document.getElementById('cc-terms').classList.contains('cc-grid'), null, { timeout: 5000 }).catch(() => {});
await sleep(300);
await page.click('#cc-grid-toggle');
await page.waitForFunction(() => !document.getElementById('cc-terms').classList.contains('cc-grid'), null, { timeout: 5000 }).catch(() => {});

// interns
await page.evaluate(() => { var t=document.getElementById('intern-toggle'); if(t) t.click(); document.getElementById('intern-section-available').open = true; });
...
const info = await page.evaluate(() => {
  ...
  return {
    ...
    pageErrorsSoFar: window.__lastErr || null,
  };
});
console.log(JSON.stringify(info, null, 2));
await page.screenshot({ path: 'brain/test/tmp3.png' });
await browser.close();
server.kill();
