...
function fleet() {
  const a = [{ id: 'roster:ceo', label: 'Operator', tier: 'ceo', model: 'human', status: 'active', parent: null }];
  ...
  return { generatedAt: new Date().toISOString(), tiers: { ceo: { name: 'Operator' } }, agents: a,
...
if (geom) {
  ...
  T('CEO subtree cost is the whole fleet', Math.abs(byId2['ceo:operator'].subtreeCost - 3.00) < 1e-6,
    `$${byId2['ceo:operator'].subtreeCost}`);
  ...
  T('tokens roll up alongside cost', byId2['ceo:operator'].subtreeTokens === 600000,
    `${byId2['ceo:operator'].subtreeTokens} tokens`);
  ...
  T('cost rolls up the full roster, not the drawn subset',
    Math.abs(byId2['ceo:operator'].subtreeCost - 3.00) < 1e-6 && nodes.filter(n => n.orgTier === 'employee').length === 10,
    `$${byId2['ceo:operator'].subtreeCost} over ${nodes.filter(n => n.orgTier === 'employee').length} drawn`);
}
...
// The row is labelled "Files" (was "Case Files") and now counts every artifact
// Alfred can see, not just vault notes. The assertion is unchanged in
// substance: the row must carry a real number, never the "--" placeholder.
T('the status rail fills when deep-linked past the brain view',
  /\bFiles\s*\d/.test(railText) && !/\bFiles\s*--/.test(railText), railText.slice(0, 160));
await deep.close();
