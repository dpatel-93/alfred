---
name: browser
description: Web browser automation with AI-optimized snapshots for alfred-flow agents
version: 1.0.0
triggers:
  - /browser
  - browse
...
npx @alfred-flow/cli memory store --namespace browser-patterns --key "login-flow" --value "snapshot->fill->click->wait"

# Retrieve before similar task
npx @alfred-flow/cli memory search --query "login automation"
```

### Hooks
```bash
# Pre-browse hook (get context)
npx @alfred-flow/cli hooks pre-edit --file "browser-task.ts"

# Post-browse hook (record success)
npx @alfred-flow/cli hooks post-task --task-id "browse-1" --success true
```
...
## Accessibility-tree-first (Claude for Chrome pattern)

As of 2026, Anthropic's Claude for Chrome and the broader Claude computer-use
work treat the **accessibility tree** (ARIA roles, names, states) as the
ground truth — not pixel screenshots. Reasons:

- AX tree is ~10–100x smaller than a screenshot, dramatically lower token cost
- Element refs are stable across renders; pixel coordinates are not
- Disabled / hidden state is explicit in AX, has to be inferred from screenshots
- Works with screen readers and assistive tooling — same surface humans use

**Defaults this skill enforces:**

- Reach for `snapshot -i` first; only take a `screenshot` when the AX tree
  doesn't capture what you need (visual layout bugs, image content, canvas)
- When debugging "agent clicked the wrong button," diff the AX snapshot
  before/after — not the screenshot
- Treat screenshots as supplemental evidence, not the primary observation

Reference: Anthropic Claude for Chrome docs (code.claude.com/docs/en/chrome).
