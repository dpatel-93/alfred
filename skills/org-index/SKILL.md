...
|---|---|---|
| `architect` | `chief-of-staff` | I own how systems are shaped — the Azure footprint, the network and identity layout, the hosting model, the IaC… |
| `cfo` | `chief-of-staff` | I own whether the data underneath everything is well-modeled, what it costs to run, and whether the trading… |
...
| `cto` | `chief-of-staff` | I own whether the product actually works for the people using it — the backend services, the frontend UI, the… |

## Managers — one per discipline

| Agent | Reports to | Owns |
|---|---|---|
| `analytics-manager` | `cfo` | I own two different questions that both land here: whether an ML model is built and tuned right, and what the… |
...
| `backend-manager` | `cto` | I own server-side implementation across every project — the code that receives a request, decides what it means,… |
| `compliance-manager` | `cso` | I own whether we can **prove** this estate is safe — not whether it is |
| `data-manager` | `cfo` | I own the data layer — schema, migrations, ingestion pipelines, and query performance — across every project that… |
| `devops-manager` | `coo` | I own whether the pipes actually work: CI/CD pipeline health across GitHub Actions and Azure DevOps Pipelines, and… |
| `docs-manager` | `cto` | I own writing documentation — API references, runbooks, READMEs, handoff docs — but only when someone explicitly… |
| `dr-manager` | `cso` | I own whether this estate can actually come back from losing something important — not whether a backup policy is… |
| `frontend-manager` | `cto` | I own whether the UI actually works and doesn't look or behave like it was thrown together — every component,… |
| `infra-manager` | `architect` | I own turning an infra requirement into a design that's actually buildable in Terraform — the module and state… |
...
| `platform-manager` | `architect` | I own where a workload actually runs — not the network under it, not the pipeline that ships it, just the platform… |
| `qa-manager` | `coo` | I own whether our tests can be trusted, not whether they exist |
| `quant-manager` | `cfo` | I own whether a trading idea is actually tradeable, not just whether it looks good |
...
| Agent | Reports to | Owns |
|---|---|---|
| `analytics-cost-eng` | `analytics-manager` | I find out what an Azure resource actually costs, trace a spend spike to the thing causing it, and lay out or… |
...
| `appsec-threat-modeler` | `appsec-manager` | I think like the attacker, not the auditor |
| `backend-api-dev` | `backend-manager` | I write and fix the app's own backend surface — endpoints, handlers, business logic, and data access — inside… |
| `backend-integration-dev` | `backend-manager` | I write and fix the code that talks to systems outside the app — third-party APIs, webhooks, auth flows, and… |
...
| `comp-evidence-collector` | `compliance-manager` | I gather the actual proof that a named control is met — a config export, a policy assignment list, a log retention… |
...
| `data-schema-eng` | `data-manager` | I own what a table looks like and how fast it answers queries — schema design, new migrations, indexing, and… |
...
| `docs-runbook-writer` | `docs-manager` | I write operational runbooks, READMEs, and handoff/close-out docs — the detailed explanation this framework's… |
...
| `frontend-ui-dev` | `frontend-manager` | I build and fix the parts of a UI you can see and touch — components, layout, styling — inside exactly the file(s)… |
| `infra-identity-eng` | `infra-manager` | I scope and document the Entra ID identity surface a service needs to authenticate — app registrations, Graph… |
...
| `infra-terraform-eng` | `infra-manager` | I check whether Terraform is actually safe to apply — module structure, state consistency, and plan output — not… |
...
| `platform-appservice-eng` | `platform-manager` | I find the right Azure PaaS home for a workload and the cheapest tier that actually covers it |
| `platform-container-eng` | `platform-manager` | I answer whether and how a workload should run in a container, on AKS, and I teach it while I do — this is a… |
| `qa-browser-tester` | `qa-manager` | I write browser tests that can actually fail, and I hunt the specific way browser suites go quietly wrong: a path… |
| `qa-test-author` | `qa-manager` | I write tests that can actually fail |
| `quant-risk-analyst` | `quant-manager` | I answer the question "how much can this actually cost me" with real math, not a feel for it |
| `quant-strategy-dev` | `quant-manager` | I write and debug the PineScript that runs on the operator's TradingView setup, and I run the first-pass backtest… |
