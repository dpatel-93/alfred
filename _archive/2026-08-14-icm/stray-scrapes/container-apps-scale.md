...
Reading modeTable of contents[Read in English](https://learn.microsoft.com/en-us/azure/container-apps/scale-app)Add to CollectionsAdd to plan[Edit](https://github.com/MicrosoftDocs/azure-docs/blob/main/articles/container-apps/scale-app.md)
...
Access to this page requires authorization. You can try [signing in](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#) or changing directories.
...
Azure CLIAzure portalARM templateBicep
...
Adding or editing scaling rules creates a new revision of your container app. A revision is an immutable snapshot of your container app. To learn which types of changes trigger a new revision, see revision [change types](https://learn.microsoft.com/en-us/azure/container-apps/revisions#change-types).

[Event-driven Container Apps jobs](https://learn.microsoft.com/en-us/azure/container-apps/jobs#event-driven-jobs) use scaling rules to trigger executions based on events.

[Section titled: Scale definition](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#scale-definition)
...
[Scale rules](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#scale-rules) are implemented as HTTP, TCP (Transmission Control Protocol), or custom.

- **Behavior** is the combination of rules and limits to determine scale decisions over time.

[Scale behavior](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#scale-behavior) explains how scale decisions are made.
...
- Replicas that aren't processing, but remain in memory might be billed at a lower "idle" rate. For more information, see [Billing](https://learn.microsoft.com/en-us/azure/container-apps/billing).
...
[Section titled: Scale rules](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#scale-rules)

## Scale rules

Three categories of triggers determine how scaling occurs:

- [HTTP](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#http): Based on the number of concurrent HTTP requests to your revision.
- [TCP](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#tcp): Based on the number of concurrent TCP connections to your revision.
- [Custom](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#custom): Based on custom metrics like:
...
[Section titled: Example](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#example)
...
Define an HTTP scale rule using the `--scale-rule-http-concurrency` parameter in the [`create`](https://learn.microsoft.com/en-us/cli/azure/containerapp#az-containerapp-create) or [`update`](https://learn.microsoft.com/en-us/cli/azure/containerapp#az-containerapp-update) commands.
...
5. Select the minimum and maximum replica range.

![Screenshot of Azure Container Apps scale range slider.](https://learn.microsoft.com/en-us/azure/container-apps/media/scale-app/azure-container-apps-scale-slide.png)

6. Select **Add**.

7. In the _Rule name_ box, enter a rule name.

8. From the _Type_ dropdown, select **HTTP Scaling**.

9. In the _Concurrent requests_ box, enter your desired number of concurrent requests for your container app.


[Section titled: TCP](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#tcp)

## TCP

With a TCP scaling rule, you have control over the threshold of concurrent TCP connections that determines how your app scales. Every 15 seconds, the number of concurrent connections is calculated as the number of connections in the past 15 seconds divided by 15. [Container Apps jobs](https://learn.microsoft.com/en-us/azure/container-apps/jobs) don't support TCP scaling rules.
...
[Section titled: Example](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#example-1)
...
Define a TCP scale rule using the `--scale-rule-tcp-concurrency` parameter in the [`create`](https://learn.microsoft.com/en-us/cli/azure/containerapp#az-containerapp-create) or [`update`](https://learn.microsoft.com/en-us/cli/azure/containerapp#az-containerapp-update) commands.
...
Not supported in the Azure portal. Use the [Azure CLI](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#tcp), [Azure Resource Manager](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?&pivots=azure-resource-manager#tcp), or [Bicep](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?&pivots=container-apps-bicep#tcp) to configure a TCP scale rule.

[Section titled: Custom](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#custom)
...
For [event-driven Container Apps jobs](https://learn.microsoft.com/en-us/azure/container-apps/jobs#event-driven-jobs), you can create a custom scaling rule based on any [ScaledJob](https://keda.sh/docs/latest/concepts/scaling-jobs/)-based KEDA scalers.

The following example demonstrates how to create a custom scale rule.

[Section titled: Example](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#example-2)
...
For authentication, KEDA scaler authentication parameters take [Container Apps secrets](https://learn.microsoft.com/en-us/azure/container-apps/manage-secrets) or [managed identity](https://learn.microsoft.com/en-us/azure/container-apps/managed-identity#scale-rules).
...
2. In the Bicep template, enter the scaler `type` value into the `custom.type` property of the scale rule.
...
4. In the Bicep template, add all metadata values to the `custom.metadata` section of the scale rule.
...
[Section titled: Authentication](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#authentication)
...
[Section titled: Use secrets](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#use-secrets)
...
2. In the `TriggerAuthentication` object, find each `secretTargetRef` and its associated secret.
...
3. In the Bicep template, for each secret:


1. Add a [secret](https://learn.microsoft.com/en-us/azure/container-apps/manage-secrets) to the container app's `secrets` array containing the secret name and value.

2. Add an entry to the `auth` array of the scale rule.

      1. Set the value of the `triggerParameter` property to the value of the `secretTargetRef`'s `parameter` property.

      2. Set the value of the `secretRef` property to the name of the `secretTargetRef`'s `key` property.
...
Refer to the [considerations section](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#considerations) for more security related information.

[Section titled: Using managed identity](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#using-managed-identity)
...
To learn more about using managed identity with scale rules, see [Managed identity](https://learn.microsoft.com/en-us/azure/container-apps/managed-identity#scale-rules).
...
4. In the ARM template, add all metadata values to the `custom.metadata` section of the scale rule.
...
[Section titled: Authentication](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#authentication-1)

### Authentication

Container Apps scale rules support secrets-based authentication. Scale rules for Azure resources, including Azure Queue Storage, Azure Service Bus, and Azure Event Hubs, also support managed identity. Where possible, use managed identity authentication to avoid storing secrets within the app.

[Section titled: Use secrets](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#use-secrets-1)
...
3. In the ARM template, for each secret:
...
[Section titled: Using managed identity](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#using-managed-identity-1)
...
[Section titled: Authentication](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#authentication-2)

### Authentication

Container Apps scale rules support secrets-based authentication. Scale rules for Azure resources, including Azure Queue Storage, Azure Service Bus, and Azure Event Hubs, also support managed identity. Where possible, use managed identity authentication to avoid storing secrets within the app.

[Section titled: Use secrets](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#use-secrets-2)
...
2. In your container app, create the [secrets](https://learn.microsoft.com/en-us/azure/container-apps/manage-secrets) that match the `secretTargetRef` properties.
...
2. Create an authentication entry with the `--scale-rule-auth` parameter. If there are multiple entries, separate them with a space.
...
[Section titled: Using managed identity](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#using-managed-identity-2)
...
01. Go to your container app in the Azure portal.

02. Select **Scale**.

03. Select **Edit and deploy**.

04. Select the **Scale and replicas** tab.

05. Select the minimum and maximum replica range.

    ![Screenshot of Azure Container Apps scale range slider.](https://learn.microsoft.com/en-us/azure/container-apps/media/scale-app/azure-container-apps-scale-slide.png)

06. Select **Add**.

07. In the _Rule name_ box, enter a rule name.

08. From the _Type_ dropdown, select **Custom**.

09. From the KEDA scaler specification, find the `type` value.
...
10. In the _Custom rule type_ box, enter the scaler `type` value.

11. From the KEDA scaler specification, find the `metadata` values.
...
12. In the portal, find the _Metadata_ section and select **Add**. Enter the name and value for each item in the KEDA `ScaledObject` specification metadata section.


[Section titled: Authentication](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#authentication-3)

### Authentication

Container Apps scale rules support secrets-based authentication. Scale rules for Azure resources, including Azure Queue Storage, Azure Service Bus, and Azure Event Hubs, also support managed identity. Where possible, use managed identity authentication to avoid storing secrets within the app.

[Section titled: Use secrets](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#use-secrets-3)

#### Use secrets

1. In your container app, create the [secrets](https://learn.microsoft.com/en-us/azure/container-apps/manage-secrets) that you want to reference.

2. Find the `TriggerAuthentication` object referenced by the KEDA `ScaledObject` specification. Identify each `secretTargetRef` of the `TriggerAuthentication` object.
...
3. In the _Authentication_ section, select **Add** to create an entry for each KEDA `secretTargetRef` parameter.


[Section titled: Using managed identity](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#using-managed-identity-3)

#### Using managed identity

Managed identity authentication isn't supported in the Azure portal. Use the [Azure CLI](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#authentication) or [Azure Resource Manager](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?&pivots=azure-resource-manager#authentication) to authenticate using managed identity.

[Section titled: Default scale rule](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#default-scale-rule)
...
[Section titled: Scale behavior](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#scale-behavior)
...
[Section titled: Example](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#example-3)
...
[Section titled: Considerations](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#considerations)
...
- No usage charges are incurred when an application scales to zero. For more pricing information, see [Billing in Azure Container Apps](https://learn.microsoft.com/en-us/azure/container-apps/billing).

- You need to enable data protection for all .NET apps on Azure Container Apps. See [Deploying and scaling an ASP.NET Core app on Azure Container Apps](https://learn.microsoft.com/en-us/aspnet/core/host-and-deploy/scaling-aspnet-apps/scaling-aspnet-apps) for details.


[Section titled: Known limitations](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#known-limitations)
...
[Section titled: Next steps](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#next-steps)
...
[Scale and manage deployed container apps - Training](https://learn.microsoft.com/en-us/training/modules/scale-manage-deployed-container-apps/?source=recommendations)
...
[Sign in](https://learn.microsoft.com/en-us/azure/container-apps/scale-app?pivots=azure-cli#)